...
function videosHandler(state, { type, payload }) {
    ...
    switch (type) {
        ...
        case 'REMOVE_FROM_LIKED_VIDEOS':
            return { ...state, likedVideos: likedVideos.filter(video => video.id !== payload.video.id) }
        case 'ADD_NEW_PLAYLIST':
            return { ...state, playlists: [...playlists, { name: payload.name, list: [] }] }
        case 'REMOVE_PLAYLIST':
            return { ...state, playlists: playlists.filter(playlist => playlist.name !== payload.playlist.name) }
        case 'ADD_TO_PLAYLIST':
            ...
            // update the playlist
            tempPlaylists[index] = { ...tempPlaylists[index], list: [...tempPlaylists[index].list, payload.video] }
